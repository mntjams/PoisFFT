program benchmark
  ...
  if (argc < 10) then
    write(0,*) "Usage: ./benchmark_mpi float_precision solver_dim &
        use_nonuniform_z measurement_iters grid_side_size_begin &
        grid_side_size_end grid_side_size_step out_file nps_y nps_z BC_1 BC_2 ..."
    stop 1
  end if
  ...
  ! PoisFFT_InitMPIGrid expects the nps with slowest growing dim first
  call get_command_argument(9, arg)
  read(arg, *) nps(2) ! nps_y
  
  call get_command_argument(10, arg)
  read(arg, *) nps(1) ! nps_z
  
  
  if (argc /= 10 + 2 * solver_dim) then
  ...
  bc_name = ""
  do i = 1, 2*solver_dim
    call get_command_argument(10 + i, arg)
  ...
  if (rank == 0) then
    ...
    write(10, '(A)') "dim,precision,solver_type,api,bc_name,num_procs,grid_size,iteration,total_execution_ms"
    flush(10)
  end if
  ...
  if (trim(float_precision) == "float") then
    call run_benchmarks_sp(solver_dim, use_nonuniform_z, measurement_iters, &
        grid_size, grid_size_end, grid_size_step, BCs, trim(bc_name), nps)
...
contains
  ...
  subroutine run_benchmarks_dp(dim, use_nonuniform_z, measurement_iters, &
          grid_size, grid_size_end, grid_size_step, BCs, bc_name, nps)
    ...
    integer(c_int), intent(in) :: nps(2)
    integer :: rank, num_procs, cart_comm, ie
    integer(c_intptr_t) :: ng(3), local_ni(3), local_i_start(3), local_no(3), local_o_start(3)
    real(c_double) :: worst_time
    ...
    do while(size <= grid_size_end)
      ...
      call MPI_Comm_rank(MPI_COMM_WORLD, rank, ie)
      call MPI_Comm_size(MPI_COMM_WORLD, num_procs, ie)
      call PoisFFT_InitMPIGrid(MPI_COMM_WORLD, nps, cart_comm, ie)
      call PoisFFT_LocalGridSize(dim, ng, cart_comm, local_ni, local_i_start, local_no, local_o_start)
      
      nx = int(local_ni(1))
      ny = int(local_ni(2))
      nz = int(local_ni(3))

      if (rank == 0) print *, "Measuring grid size ", size
      ...
      if (dim == 1) then
        S1 = PoisFFT_Solver1D_DP([nx], [lenx], BCs, gnxyz=int(ng(1:1)), &
            offs=int(local_i_start(1:1)), mpi_comm=cart_comm)
      else if (dim == 2) then
        S2 = PoisFFT_Solver2D_DP([nx, ny], [lenx, leny], BCs, &
            gnxyz=int(ng(1:2)), offs=int(local_i_start(1:2)), mpi_comm=cart_comm)
      else if (dim == 3) then
        if (use_nonuniform_z) then
          allocate(z(0:size+1))
          allocate(z_u(-1:size+1))
          z_u(0:size) = [(lenz/size*i, i = 0, size)]
          do i = 1, size-1
            ...
            z_u(size+1) = z_u(size) + (z_u(size)-z_u(size-1))
          do i = 0, size+1
            z(i) = (z_u(i)+z_u(i-1))/2.0_c_double
          end do
          S3_nz = PoisFFT_Solver3D_nonuniform_z_DP([nx,ny,nz], [lenx, leny], &
              z(1:size), z_u(0:size), BCs, gnxyz=int(ng(1:3)), &
              offs=int(local_i_start(1:3)), mpi_comm=cart_comm)
        else
          S3 = PoisFFT_Solver3D_DP([nx, ny, nz], [lenx, leny, lenz], BCs, &
              gnxyz=int(ng(1:3)), offs=int(local_i_start(1:3)), mpi_comm=cart_comm)
        end if
      end if
      
      do it = 1, measurement_iters
        if (rank == 0) print *, "    Iteration ", it-1
        if (dim == 1) then
!$omp parallel do
          do i = 1, nx
            call random_number(RHS1D(i))
            RHS1D(i) = RHS1D(i) + rank
...
!$omp parallel do collapse(3)
        ...
        time = real(t2 - t1, c_double) / real(count_rate, c_double) * 1000.0_c_double
        
        call MPI_Reduce(time, worst_time, 1, MPI_DOUBLE_PRECISION, MPI_MAX, 0, cart_comm, ie)
        
        if (rank == 0) then
            write(10, '(I0,6A,I0,A,I0,A,I0,A,F15.6)') dim, ',double', &
               trim(merge(',nonuniform_z', ',uniform     ', use_nonuniform_z)), ',PoisFFT,', &
               '"', trim(bc_name), '",', num_procs, ',', size, ',', (it-1), ',', worst_time
          flush(10)
        end if
      end do
  ...
  subroutine run_benchmarks_sp(dim, use_nonuniform_z, measurement_iters, &
          grid_size, grid_size_end, grid_size_step, BCs, bc_name, nps)
    integer, intent(in) :: dim, measurement_iters, grid_size, grid_size_end, grid_size_step
    logical, intent(in) :: use_nonuniform_z
    integer, intent(in) :: BCs(2*dim)
    ...
    do while(size <= grid_size_end)
      ...
      if (dim == 1) then
        S1 = PoisFFT_Solver1D_SP([nx], [lenx], BCs, gnxyz=int(ng(1:1)), &
            offs=int(local_i_start(1:1)), mpi_comm=cart_comm)
      else if (dim == 2) then
        ...
        if (use_nonuniform_z) then
          ...
          S3_nz = PoisFFT_Solver3D_nonuniform_z_SP([nx,ny,nz], [lenx, leny], &
              z(1:size), z_u(0:size), BCs, gnxyz=int(ng(1:3)), &
              offs=int(local_i_start(1:3)), mpi_comm=cart_comm)
        else
          S3 = PoisFFT_Solver3D_SP([nx, ny, nz], [lenx, leny, lenz], &
              BCs, gnxyz=int(ng(1:3)), offs=int(local_i_start(1:3)), mpi_comm=cart_comm)
        end if
      end if
...
!$omp parallel do collapse(3)
        ...
        if (rank == 0) then
          write(10, '(I0,6A,I0,A,I0,A,I0,A,F15.6)') dim, ',float', &
               trim(merge(',nonuniform_z', ',uniform     ', use_nonuniform_z)), ',PoisFFT,', &
               '"', trim(bc_name), '",', num_procs, ',', size, ',', (it-1), ',', worst_time
          flush(10)
        end if
